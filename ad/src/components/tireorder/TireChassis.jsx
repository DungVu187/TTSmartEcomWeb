...
const TWELVE_REAR_LABEL_LAYOUT = {
  ...
  rear_left_aft_outer: { path: 'M 115 251 L 103 256', x: 63, y: 256 },
  rear_left_aft_inner: { path: 'M 149 273 L 124 308 H 108', x: 68, y: 308 },
  ...
  rear_right_aft_inner: { path: 'M 291 273 L 316 308 H 332', x: 372, y: 308 },
  rear_right_aft_outer: { path: 'M 325 251 L 337 256', x: 377, y: 256 },
};
...
const LABEL_LAYOUTS = {
  ...
  12: {
    front_left: { path: 'M 138 53 L 108 48', x: 68, y: 48 },
    front_right: { path: 'M 302 53 L 332 48', x: 372, y: 48 },
    front_second_left: { path: 'M 138 113 L 108 100', x: 68, y: 100 },
    front_second_right: { path: 'M 302 113 L 332 100', x: 372, y: 100 },
    ...
    <svg className="tire-link-layer" viewBox="0 0 440 350" aria-hidden="true">
      ...
      {labelledAssignments.map(({ slotId, assignment }) => {
        ...
        return <g key={slotId}>
          ...
          <text className="tire-product-code" x={label.x} y={label.y - 18.75} textAnchor="middle">{assignment.productCodeSnapshot}</text>
          <text className="tire-product-meta" x={label.x} y={label.y - 6.25} textAnchor="middle">Mã lốp: {assignment.serialNumber || '—'}</text>
          <text className="tire-product-meta" x={label.x} y={label.y + 6.25} textAnchor="middle">Thay: {replacementDate}</text>
          <text className="tire-product-meta" x={label.x} y={label.y + 18.75} textAnchor="middle">Đã dùng: {usageDuration}</text>
        </g>;
      })}
    </svg>
