...
const TireLifecycleTimelineItem = ({ item, index, navigate }) => <Box className="tire-lifecycle-timeline-item">
  ...
  <Paper variant="outlined" className="tire-lifecycle-timeline-card">
    <Box className="tire-lifecycle-timeline-card-head"><Chip size="small" label={`Lần ${index + 1}`} /><Box><Typography>Mã sản phẩm</Typography><strong>{item.productCode || '—'}</strong></Box><Box><Typography>Mã lốp</Typography><strong>{item.serialNumber || '—'}</strong></Box><Box><Typography>Tên lốp</Typography><strong>{item.productName || '—'}</strong></Box><Stack direction="row" spacing={0.75} alignItems="center"><StatusChip status={item.status} />{item.installOrderDeleted && <Chip size="small" color="error" label="Đơn đã xóa" />}</Stack></Box>
  ...
  return <Box className="tire-lifecycle-page">
    <Paper className="tire-lifecycle-filter-card">
      ...
      <Box className="tire-lifecycle-filter-grid">
        ...
        <Autocomplete freeSolo autoHighlight options={data.suggestions?.tireCodes || []} inputValue={filters.tire} onInputChange={(_, value) => setFilters({ ...filters, tire: value })} renderInput={(params) => <TextField {...params} label="Mã sản phẩm" placeholder="Nhập mã sản phẩm..." InputProps={{ ...params.InputProps, startAdornment: <><InputAdornment position="start"><LocalOfferOutlinedIcon /></InputAdornment>{params.InputProps.startAdornment}</> }} />} />
