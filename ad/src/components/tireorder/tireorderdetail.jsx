      ...
      <DialogContent>
        <Stack spacing={2} mt={1}>
          ...
          {Array.from({ length: editingAssignment ? 1 : Number(quantity) }, (_, index) => {
            ...
            return <TextField key={slotId || `empty-${index}`} required disabled={!slotId} label={Number(quantity) > 1 ? `Seri lốp ${index + 1}${positionNumber ? ` — Vị trí ${positionNumber}` : ''}` : 'Seri lốp'} value={value} onChange={(event) => { setSerialNumbersBySlot((current) => ({ ...current, [slotId]: event.target.value })); setSerialErrorsBySlot((current) => ({ ...current, [slotId]: '' })); }} error={Boolean(serverError || duplicated)} helperText={!slotId ? 'Chọn vị trí lốp bên dưới để nhập seri.' : serverError || (duplicated ? 'Seri không được trùng trong cùng lần thêm.' : 'Seri của từng chiếc lốp, không lưu vào sản phẩm.')} FormHelperTextProps={serverError ? { sx: { fontSize: '0.875rem', fontWeight: 500 } } : undefined} />;
          })}
          ...
          {tiresBeingStopped.map((tire) => <Alert key={tire.slotId} severity="warning" variant="outlined">
            <Typography sx={{ mb: 1 }}>Vị trí {tire.positionNumber} đang có lốp cũ: <strong>{tire.productCode || '—'} — {tire.productName}</strong>{tire.serialNumber ? <> — Seri: <strong>{tire.serialNumber}</strong></> : null} - Ngày thay trước: <strong>{dateText(tire.startedAt)}</strong></Typography>
