...
const TireOrderDetail = () => {
  ...
  const [order, setOrder] = useState(null); const [selectedVehicleId, setSelectedVehicleId] = useState(''); const [vehicleDialog, setVehicleDialog] = useState(false); const [vehicleToDelete, setVehicleToDelete] = useState(null); const [vehicleDeleteLoading, setVehicleDeleteLoading] = useState(false); const [assignmentDialog, setAssignmentDialog] = useState(false); const [vehicles, setVehicles] = useState([]); const [vehicleFilter, setVehicleFilter] = useState('all'); const [vehicleWheelCount, setVehicleWheelCount] = useState(10); const [products, setProducts] = useState([]); const [product, setProduct] = useState(null); const [variantIndex, setVariantIndex] = useState(''); const [quantity, setQuantity] = useState(1); const [slotIds, setSlotIds] = useState([]); const [activeTires, setActiveTires] = useState([]); const [activeTiresLoading, setActiveTiresLoading] = useState(false); const [stoppedAtBySlot, setStoppedAtBySlot] = useState({}); const [highlighted, setHighlighted] = useState(null); const [editingAssignment, setEditingAssignment] = useState(null); const [performedAt, setPerformedAt] = useState(''); const [assignmentNote, setAssignmentNote] = useState(''); const [replacement, setReplacement] = useState(null); const [newVehiclePlate, setNewVehiclePlate] = useState(''); const [newVehicleName, setNewVehicleName] = useState(''); const [confirmationAction, setConfirmationAction] = useState(null); const [confirmationLoading, setConfirmationLoading] = useState(false);
  ...
  const saveAssignment = async () => {
    ...
    if (!product || !variant || (!editingAssignment && slotIds.length !== Number(quantity)) || oldTireSlots.some((slotId) => !stoppedAtBySlot[slotId])) return;
    ...
    if (editingAssignment) body.previousTireStoppedAt = stoppedAtBySlot[editingAssignment.slotId] || null;
    else body.previousTireStoppedAtBySlot = Object.fromEntries(slotIds.filter((slotId) => stoppedAtBySlot[slotId]).map((slotId) => [slotId, stoppedAtBySlot[slotId]]));
    try { const result = editingAssignment ? await updateTireAssignment(id, selectedVehicleId, editingAssignment._id, body) : await addTireAssignments(id, selectedVehicleId, { ...body, quantity: Number(quantity), slotIds }); apply(result.order); setAssignmentDialog(false); setEditingAssignment(null); } catch (error) { toast.error(error.message); }
  };
  ...
  return <Box className="inventory-order-detail-page tire-order-detail" p={2}>
    <Paper className="tire-order-info">
      <Box className="tire-order-info-head"><Typography variant="h5">Thông tin đơn</Typography><Stack direction="row" spacing={1}>{editable && <Button variant="contained" startIcon={<CheckCircleIcon />} onClick={() => setConfirmationAction('complete')}>Hoàn thành đơn</Button>}{order.status === 'completed' && can('tireorder.edit') && <Button variant="outlined" onClick={() => setConfirmationAction('revert')}>Hủy hoàn thành</Button>}{can('tireorder.delete') && <Button color="error" variant="outlined" onClick={() => setConfirmationAction('delete')}>Xóa đơn</Button>}</Stack></Box>
      ...
      <DialogContent>
        <Stack spacing={2} mt={1}>
          ...
          {tiresBeingStopped.map((tire) => <Alert key={tire.slotId} severity="warning" variant="outlined">
            <Typography sx={{ mb: 1 }}>Vị trí {tire.positionNumber} đang có lốp cũ: <strong>{tire.productCode || '—'} — {tire.productName}</strong> - Ngày thay trước: <strong>{dateText(tire.startedAt)}</strong></Typography>
          ...
          {!editingAssignment && <>
            ...
            {activeTiresLoading ? <Typography color="text.secondary">Đang kiểm tra lốp cũ trên xe...</Typography> : <TireChassis assignments={selectedVehicle?.assignments || []} wheelCount={selectedVehicle?.wheelCount || 10} selectedSlotIds={slotIds} selectable onSlotClick={(slotId) => toggleSlot(slotId)} />}
          </>}
        </Stack>
      </DialogContent>
      <DialogActions><Button onClick={() => setAssignmentDialog(false)}>Hủy</Button><Button variant="contained" disabled={activeTiresLoading || !product || variantIndex === '' || hasInvalidStoppedAt || (!editingAssignment && slotIds.length !== Number(quantity))} onClick={saveAssignment}>Lưu</Button></DialogActions>
    </Dialog>
